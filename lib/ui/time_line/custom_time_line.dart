...
/// The state of a [TimeLineModel] which is used to control the style of the circle and
/// text.
///
/// See also:
///
...
class CustomStepper extends StatefulWidget {
  ...
  CustomStepper({
    Key key,
    @required this.steps,
    this.type = TimeLineType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
    this.controlsBuilder,
    this.tlColor = Colors.green
  }) : assert(steps != null),
        assert(type != null),
  ...
  /// [TimeLineType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [TimeLineType.vertical] case where it is
...
class _StepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  final Map<int, TimeLineM> _oldStates = <int, TimeLineM>{};
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    final TimeLineM state = oldState ? _oldStates[index] : widget.steps[index].state;
    final bool isDarkActive = _isDark() && widget.steps[index].isActive;
    assert(state != null);
    switch (state) {
      case TimeLineM.indexed:
      case TimeLineM.disabled:
        return Text(
          '${index + 1}',
          style: isDarkActive ? _kStepStyle.copyWith(color: Colors.black87) : _kStepStyle,
        );
      case TimeLineM.editing:
        return Icon(
          Icons.directions_car,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case TimeLineM.complete:
        return Icon(
          Icons.check,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case TimeLineM.error:
...
//        child: Center(
//          child: _buildCircleChild(index, oldState && widget.steps[index].state == TimeLineM.error),
//        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == TimeLineM.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != TimeLineM.error)
    ...
    for (int i = 0; i < widget.steps.length; i += 1) {
      children.add(
          Column(
              ...
              children: <Widget>[
                InkWell(
                    onTap: widget.steps[i].state != TimeLineM.disabled ? () {
...
//            margin: const EdgeInsets.symmetric(horizontal: 24.0),
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.type) {
      case TimeLineType.vertical:
        return _buildVertical();
      case TimeLineType.horizontal:
//        return Container(
//          height: 70,
//          child: Text('eeee'),
//        );
